...
if [[ ! -f "${ENV_FILE}" ]]; then
  ...
  echo "已生成 ${ENV_FILE}，先把域名、密码和服务列表改对，再重新执行。"
  exit 1
fi
...
mkdir -p \
  "${download_dir}" \
  "${INSTALL_ROOT}/prometheus/app" \
  "${INSTALL_ROOT}/grafana/app"

prom_tar="${download_dir}/$(basename "${PROMETHEUS_URL}")"
grafana_tar="${download_dir}/$(basename "${GRAFANA_URL}")"
...
download_file() {
  ...
  if [[ "${ALLOW_DOWNLOAD}" != "true" ]]; then
    ...
    echo "请先把离线包 ${filename} 放到 ${bundle_dir}/，或设置 ALLOW_DOWNLOAD=true"
    exit 1
  fi
...
download_file "${PROMETHEUS_URL}" "${prom_tar}" "${PROMETHEUS_SHA256:-}"
download_file "${GRAFANA_URL}" "${grafana_tar}" "${GRAFANA_SHA256:-}"

"${SCRIPT_DIR}/stop.sh" --env-file "${ENV_FILE}" || true

rm -rf "${INSTALL_ROOT}/prometheus/app"/* "${INSTALL_ROOT}/grafana/app"/*
tar -xzf "${prom_tar}" -C "${INSTALL_ROOT}/prometheus/app" --strip-components=1
tar -xzf "${grafana_tar}" -C "${INSTALL_ROOT}/grafana/app" --strip-components=1

"${SCRIPT_DIR}/start.sh" --env-file "${ENV_FILE}"

echo
echo "部署完成。"
echo "平台入口应配置为: 容器内监听端口=${GRAFANA_PORT}, 用途=HTTP, 路径=${GRAFANA_SUBPATH}"
