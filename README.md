...
对 `/metrics` 计算较重、容易短时超时的特殊服务，可以单独延长 `VLLMTargetDown` 判定时间。未覆盖的服务仍保持默认 `2m`：

```bash
--target-down-for qwen3_opd_logprobs32_host1 5m \
--target-down-for qwen3_opd_logprobs32_host2 5m \
--target-down-for qwen3_opd_logprobs32_host3 5m
```
...
| 告警 | 条件 | 持续时间 |
|---|---|---|
| `VLLMTargetDown` | Prometheus 抓不到服务 `/metrics`，即 `up == 0` | 默认 `2m`，可用 `--target-down-for <service> <duration>` 按服务覆盖 |
